...
const CaseDatox = () => {
    ...
    return (
        <div className="bg-[#F5F3EE] text-foreground min-h-screen">
            ...
            <section className="px-8 lg:px-16 py-28 bg-[#D4D0C8]">
                {/* Built to How It Was Made's shape: the heading alone in a 280px rail,
                    a wide gap, the prose in the right column. The eyebrow that used to
                    sit above the heading is gone — that section has none, and the
                ...
                >
                    ...
                    <div>
                        <p className="text-base font-light text-gray-600 leading-relaxed mb-6 max-w-xl">
                        ...
                        <Link
                            to="/method"
                            className="group mt-10 inline-flex items-center gap-2 text-sm font-medium text-[#141414] hover:gap-3 transition-all"
                        >
            ...
            {/* Key challenge — How It Was Made's shape again, and for the same reason.
                A lone tinted card floating in one half of the page was the only card
                on a page that otherwise reads as hairline rows throughout, which is
                what made it look like a fragment rather than a section. Problem,
                Solution and Result are now rows in that rhythm.

                The letter that used to label the card is gone with it: there is one
                challenge, not a set, and an "A" with no B reads as a list missing its
                other items rather than a considered single case. */}
            <section className="px-8 lg:px-16 py-28">
                <motion.div
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    ...
                    <div>
                        {challenges.map((challenge) => (
                            <div key={challenge.title}>
                                <h3 className="text-2xl font-normal text-[#141414] mb-1">{challenge.title}</h3>
                                <p className="text-sm text-gray-400 mb-10">{challenge.subtitle}</p>

                                <div className="space-y-0">
                                    {[
                                        { label: "Problem", body: challenge.problem },
                                        { label: "Solution", body: challenge.solution },
                                        { label: "Result", body: challenge.result },
                                    ].map((row, index) => (
                                        <motion.div
                                            key={row.label}
                                            initial={{ opacity: 0, y: 20 }}
                                            ...
                                            viewport={{ once: true }}
                                            transition={{ delay: index * 0.1, duration: 0.6 }}
                                            className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-2 md:gap-8 py-6 border-t border-gray-300"
                                        >
                                            <p className="text-base font-semibold text-[#141414]">{row.label}</p>
                                            <p className="text-base font-light text-gray-600 leading-relaxed">{row.body}</p>
                                        </motion.div>
                                    ))}
                                </div>
                            </div>
